/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   STAN: Etap 1 DataBase KOMPLET — HQ(12)+PR(1)+EL(6)+FA(1)+HS(1)+DT(4)+TR(7) = 32. Dalej: heresy-weapons.js (slownik broni), heresy-rules.js (glosariusz), potem Builder.
============================================================================ */
...
function termCmdSquad(o){
  return {
    ...
    options:[
      ...
      { id:'chosen_combi', label:'Any Chosen may exchange combi-bolter for one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['ranged'],
        note:'Dotyczy Chosen.',
        choices:[
      ...
      { id:'power_swap', label:'Any model may exchange power weapon for one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['melee'],
        choices:[
      ...
      { id:'dual_claws', label:'Any Chosen may exchange combi-bolter AND power weapon for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['ranged','melee'],
        note:'Dotyczy Chosen.',
...
function termSquad(o){
  return {
    ...
    options:[
      ...
      { id:'special', label:'For every five models, one may exchange combi-bolter for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1}, pools:['ranged'],
        choices:[
      ...
      { id:'combi_swap', label:'Any model may exchange combi-bolter for one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['ranged'],
        choices:[
          {id:'magna_combi', name:'Magna combi-weapon', cost:10, costMode:'per-each'},
          {id:'minor_combi', name:'Minor combi-weapon', cost:5,  costMode:'per-each'},
          {id:'volkite_charger', name:'Volkite charger', cost:2,  costMode:'per-each'},
      ...
      { id:'dual_claws', label:'Any model may exchange combi-bolter AND power weapon for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['ranged','melee'],
        choices:[ {id:'two_lightning_claws', name:'Two lightning claws', cost:10, costMode:'per-each'} ]},
...
const HERESY_UNITS = [
  ...
  /* ── HQ · Legion Command Squad (power armour) ────────────────────────────── */
  {
    ...
    options:[
      ...
      { id:'bolter_swap', label:'Any model may exchange bolter for one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['ranged'],
        choices:[
      ...
      { id:'melee_swap', label:'Any model may exchange chainsword and/or bolt pistol for one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['melee'],
      ...
      { id:'dual_claws', label:'Any Legion Chosen may exchange bolt pistol AND chainsword for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['melee'],
        note:'Dotyczy Chosen; nie z bike/jetbike.',
        choices:[ {id:'two_lightning_claws', name:'Two lightning claws', cost:10, costMode:'per-each'} ]},
      { id:'nuncio', label:'Any model may select (per model):', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[ {id:'nuncio_vox', name:'Nuncio-vox', cost:5, costMode:'per-each'} ]},
      { id:'combat_shield', label:'Any model may select (per model):', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
  ...
  /* ── EL · Legion Veteran Squad ───────────────────────────────────────────── */
  {
    ...
    options:[
      ...
      { id:'pistol_swap', label:'Any model may exchange bolt pistol for one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['pistol'],
        choices:[
      ...
      { id:'dual_claws', label:'Any model may exchange bolter AND bolt pistol for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1}, pools:['ranged','pistol'],
        choices:[ {id:'two_lightning_claws', name:'Two lightning claws', cost:10, costMode:'per-each'} ]},
      { id:'special', label:'For every five models, one may exchange bolter for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1}, pools:['ranged'],
        choices:[
  ...
  /* ── TR · Legion Tactical Squad ──────────────────────────────────────────── */
  {
    id:'legion_tactical_squad', name:'Legion Tactical Squad', slot:'TR', baseCost:100,
    profileType:'model', composition:{start:10, min:10, max:20},
    profiles:[
      {name:'Legionary',               M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:1, Ld:7, Sv:'3+', Inv:'—', base:'32mm'},
      {name:'Legion Tactical Sergeant', M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:2, Ld:8, Sv:'3+', Inv:'—', base:'32mm'},
    ],
    wargear:['Bolter','Bolt pistol','Power armour','Frag grenades','Krak grenades'],
    unitType:['Legionary: Infantry (Line)','Sergeant: Infantry (Character, Line)'],
    rules:['Legiones Astartes (Space Wolves)','Fury of the Legion','Heart of the Legion'],
    rulesText:[],
    transportNote:'≤10 modeli: Legion Rhino jako Dedicated Transport (nie zużywa slotu FOC; koszt płatny).',
    options:[
      { id:'extra', label:'May include up to 10 additional Legionaries:', mode:'add-models', scope:'unit', min:0, max:10,
        choices:[ {id:'legionary', name:'Legionary', cost:10, costMode:'per-model'} ]},
      { id:'nuncio', label:'One Legionary may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'nuncio_vox', name:'Nuncio-vox', cost:10, costMode:'flat'} ]},
      { id:'augury', label:'One Legionary may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'augury_scanner', name:'Augury scanner', cost:10, costMode:'flat'} ]},
      { id:'vexilla', label:'One Legionary may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'legion_vexilla', name:'Legion vexilla', cost:10, costMode:'flat'} ]},
      { id:'bayonet', label:'Any model with a bolter may take one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[
          {id:'bayonet',       name:'Bayonet',       cost:1, costMode:'per-each'},
      ...
      { id:'chainswords', label:'All Legionaries may be given chainswords:', mode:'unit-per-model', scope:'each-model',
        choices:[ {id:'chainswords', name:'Chainswords (whole unit)', cost:5, costMode:'per-model'} ]},
      { id:'sgt_bolter', label:'The Sergeant may exchange bolter for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'magna_combi', name:'Magna combi-weapon', cost:10, costMode:'flat'},
          {id:'minor_combi', name:'Minor combi-weapon', cost:5,  costMode:'flat'},
        ]},
      { id:'sgt_melee', label:'The Sergeant may take one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'chainsword',      name:'Chainsword',       cost:5,  costMode:'flat'},
          {id:'heavy_chainsword',name:'Heavy chainsword', cost:10, costMode:'flat'},
          {id:'power_weapon',    name:'Power weapon',     cost:10, costMode:'flat'},
          {id:'power_fist',      name:'Power fist',       cost:15, costMode:'flat'},
          {id:'lightning_claw',  name:'Lightning claw',   cost:10, costMode:'flat'},
        ]},
      { id:'sgt_pistol', label:'The Sergeant may exchange bolt pistol for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'plasma_pistol',    name:'Plasma pistol',    cost:10, costMode:'flat'},
          {id:'volkite_serpenta', name:'Volkite serpenta', cost:5,  costMode:'flat'},
          {id:'hand_flamer',      name:'Hand flamer',      cost:2,  costMode:'flat'},
        ]},
      { id:'sgt_armour', label:'The Sergeant may exchange power armour for:', mode:'single-swap', scope:'sergeant',
        choices:[ {id:'artificer_armour', name:'Artificer armour', cost:10, costMode:'flat', statMods:{Sv:'2+'}} ]},
  ...
  /* ── TR · Legion Assault Squad ───────────────────────────────────────────── */
  {
    id:'legion_assault_squad', name:'Legion Assault Squad', slot:'TR', baseCost:145,
    profileType:'model', composition:{start:10, min:10, max:20},
    profiles:[
      {name:'Legionary',              M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:1, Ld:7, Sv:'3+', Inv:'—', base:'32mm'},
      {name:'Legion Assault Sergeant',M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:2, Ld:8, Sv:'3+', Inv:'—', base:'32mm'},
    ],
    wargear:['Bolt pistol','Chainsword','Legion Warhawk jump pack','Power armour','Frag grenades','Krak grenades'],
    unitType:['Legionary: Infantry (Line)','Sergeant: Infantry (Line, Character)'],
    rules:['Legiones Astartes (Space Wolves)'],
    rulesText:[],
    options:[
      { id:'extra', label:'May include up to 10 additional Legionaries:', mode:'add-models', scope:'unit', min:0, max:10,
        choices:[ {id:'legionary', name:'Legionary', cost:12, costMode:'per-model'} ]},
      { id:'combat_shields', label:'The entire unit may take combat shields:', mode:'unit-per-model', scope:'each-model',
        choices:[ {id:'combat_shields', name:'Combat shields', cost:2, costMode:'per-model', statMods:{Inv:'6++'}} ]},
      { id:'melee_ratio', label:'For every five models, one may exchange chainsword for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1},
        choices:[
          {id:'heavy_chainsword', name:'Heavy chainsword', cost:2, costMode:'per-each'},
          {id:'power_weapon',     name:'Power weapon',     cost:5, costMode:'per-each'},
          {id:'charnabal',        name:'Charnabal weapon', cost:5, costMode:'per-each'},
        ]},
      { id:'pistol_ratio', label:'For every five models, one may exchange bolt pistol for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1},
        choices:[
          {id:'hand_flamer',      name:'Hand flamer',      cost:5,  costMode:'per-each'},
          {id:'volkite_serpenta', name:'Volkite serpenta', cost:5,  costMode:'per-each'},
          {id:'plasma_pistol',    name:'Plasma pistol',    cost:10, costMode:'per-each'},
        ]},
      { id:'sgt_swap', label:'The Sergeant may exchange bolt pistol and/or chainsword for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'plasma_pistol',    name:'Plasma pistol',    cost:10, costMode:'flat'},
          {id:'hand_flamer',      name:'Hand flamer',      cost:5,  costMode:'flat'},
          {id:'volkite_serpenta', name:'Volkite serpenta', cost:5,  costMode:'flat'},
          {id:'heavy_chainsword', name:'Heavy chainsword', cost:2,  costMode:'flat'},
          {id:'power_weapon',     name:'Power weapon',     cost:10, costMode:'flat'},
          {id:'charnabal',        name:'Charnabal weapon', cost:10, costMode:'flat'},
          {id:'power_fist',       name:'Power fist',       cost:15, costMode:'flat'},
          {id:'lightning_claw',   name:'Lightning claw',   cost:10, costMode:'flat'},
        ]},
      { id:'sgt_dual_claws', label:'The Sergeant may exchange bolt pistol AND chainsword for:', mode:'toggle', scope:'sergeant',
        choices:[ {id:'two_lightning_claws', name:'Two lightning claws', cost:15, costMode:'flat'} ]},
      { id:'sgt_bombs', label:'The Sergeant may take:', mode:'toggle', scope:'sergeant',
        choices:[ {id:'melta_bombs', name:'Melta bombs', cost:10, costMode:'flat'} ]},
      { id:'sgt_armour', label:'The Sergeant may exchange power armour for:', mode:'single-swap', scope:'sergeant',
  ...
  /* ── TR · Legion Breacher Squad ──────────────────────────────────────────── */
  {
    id:'legion_breacher_squad', name:'Legion Breacher Squad', slot:'TR', baseCost:155,
    profileType:'model', composition:{start:10, min:10, max:20},
    profiles:[
      // Inv 5++ z Boarding shield (stały wargear)
      {name:'Legion Breacher',          M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:1, Ld:7, Sv:'3+', Inv:'5++', base:'32mm'},
      {name:'Legion Breacher Sergeant', M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:2, Ld:8, Sv:'3+', Inv:'5++', base:'32mm'},
    ],
    wargear:['Bolter','Bolt pistol','Boarding shield','Power armour','Breacher charges','Frag grenades','Krak grenades'],
    unitType:['Breacher: Infantry (Heavy, Line)','Sergeant: Infantry (Heavy, Line, Character)'],
    rules:['Legiones Astartes (Space Wolves)'],
    rulesText:[],
    transportNote:'≤10 modeli: Termite Assault Drill / Land Raider Proteus jako Dedicated Transport (nie zużywa slotu FOC; koszt płatny).',
    options:[
      { id:'extra', label:'May include up to 10 additional Legion Breachers:', mode:'add-models', scope:'unit', min:0, max:10,
        choices:[ {id:'breacher', name:'Legion Breacher', cost:15, costMode:'per-model'} ]},
      { id:'nuncio', label:'One Breacher may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'nuncio_vox', name:'Nuncio-vox', cost:10, costMode:'flat'} ]},
      { id:'vexilla', label:'One Breacher may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'legion_vexilla', name:'Legion vexilla', cost:10, costMode:'flat'} ]},
      { id:'augury', label:'One Breacher may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'augury_scanner', name:'Augury scanner', cost:10, costMode:'flat'} ]},
      { id:'bolter_swap', label:'Any model may exchange bolter for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[ {id:'volkite_charger', name:'Volkite charger', cost:2, costMode:'per-each'} ]},
      { id:'special', label:'For every five models, one may exchange bolter for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1},
        choices:[
          {id:'flamer',       name:'Flamer',       cost:5,  costMode:'per-each'},
          {id:'meltagun',     name:'Meltagun',     cost:15, costMode:'per-each'},
          {id:'graviton_gun', name:'Graviton gun', cost:15, costMode:'per-each'},
          {id:'lascutter',    name:'Lascutter',    cost:20, costMode:'per-each'},
        ]},
      { id:'sgt_bolter', label:'The Sergeant may exchange bolter for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'magna_combi', name:'Magna combi-weapon', cost:10, costMode:'flat'},
      ...
      { id:'sgt_melee', label:'The Sergeant may take one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          ...
          {id:'power_fist',     name:'Power fist',     cost:15, costMode:'flat'},
          {id:'lightning_claw', name:'Lightning claw', cost:15, costMode:'flat'},
          {id:'thunder_hammer', name:'Thunder hammer', cost:20, costMode:'flat'},
        ]},
      { id:'sgt_pistol', label:'The Sergeant may exchange bolt pistol for:', mode:'single-swap', scope:'sergeant',
        choices:[ {id:'plasma_pistol', name:'Plasma pistol', cost:10, costMode:'flat'} ]},
      { id:'sgt_armour', label:'The Sergeant may exchange power armour for:', mode:'single-swap', scope:'sergeant',
  ...
  /* ── TR · Legion Scout Squad ─────────────────────────────────────────────── */
  {
    id:'legion_scout_squad', name:'Legion Scout Squad', slot:'TR', baseCost:65,
    profileType:'model', composition:{start:5, min:5, max:10},
    profiles:[
      {name:'Legion Scout',          M:8, WS:4, BS:4, S:4, T:4, W:1, I:4, A:1, Ld:7, Sv:'4+', Inv:'—', base:'25mm'},
      {name:'Legion Scout Sergeant', M:8, WS:4, BS:4, S:4, T:4, W:1, I:4, A:2, Ld:8, Sv:'4+', Inv:'—', base:'25mm'},
    ],
    wargear:['Bolter','Bolt pistol','Scout armour','Shroud bombs','Frag grenades','Krak grenades'],
    unitType:['Scout: Infantry (Light, Skirmish)','Sergeant: Infantry (Light, Skirmish, Character)'],
    rules:['Legiones Astartes (Space Wolves)','Scout','Infiltrate','Move Through Cover','Support Squad'],
    rulesText:[],
    options:[
      { id:'extra', label:'May include up to 5 additional Legion Scouts:', mode:'add-models', scope:'unit', min:0, max:5,
        choices:[ {id:'scout', name:'Legion Scout', cost:8, costMode:'per-model'} ]},
      { id:'bolter_swap', label:'Any model may exchange bolter for one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[
          {id:'nemesis_bolter',   name:'Nemesis bolter',   cost:10, costMode:'per-each'},
          {id:'chainsword',       name:'Chainsword',       cost:0,  costMode:'per-each', free:true},
          {id:'astartes_shotgun', name:'Astartes shotgun', cost:0,  costMode:'per-each', free:true},
        ]},
      { id:'nuncio', label:'One Scout may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'nuncio_vox', name:'Nuncio-vox', cost:10, costMode:'flat'} ]},
      { id:'vexilla', label:'One Scout may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'legion_vexilla', name:'Legion vexilla', cost:10, costMode:'flat'} ]},
      { id:'bayonet', label:'Any model with a bolter may take one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[
          {id:'bayonet',       name:'Bayonet',       cost:1, costMode:'per-each'},
      ...
      { id:'unit_bombs', label:'The entire unit may take:', mode:'unit-per-model', scope:'each-model',
        choices:[ {id:'melta_bombs', name:'Melta bombs', cost:5, costMode:'per-model'} ]},
      { id:'sgt_bolter', label:'The Sergeant may exchange bolter for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'magna_combi', name:'Magna combi-weapon', cost:10, costMode:'flat'},
          {id:'minor_combi', name:'Minor combi-weapon', cost:5,  costMode:'flat'},
  ...
  /* ── TR · Legion Reconnaissance Squad ────────────────────────────────────── */
  {
    id:'legion_recon_squad', name:'Legion Reconnaissance Squad', slot:'TR', baseCost:85,
    profileType:'model', composition:{start:5, min:5, max:10},
    profiles:[
      {name:'Recon Legionary',       M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:1, Ld:7, Sv:'3+', Inv:'—', base:'32mm'},
      {name:'Legion Recon Sergeant', M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:2, Ld:8, Sv:'3+', Inv:'—', base:'32mm'},
    ],
    wargear:['Bolter','Bolt pistol','Power armour','Shroud bombs','Frag grenades','Krak grenades'],
    unitType:['Recon Legionary: Infantry (Skirmish, Line)','Sergeant: Infantry (Skirmish, Line, Character)'],
    rules:['Legiones Astartes (Space Wolves)','Scout','Infiltrate','Support Squad'],
    rulesText:[],
    transportNote:'Rhino / Storm Eagle Gunship jako Dedicated Transport (nie zużywa slotu FOC; koszt płatny).',
    options:[
      { id:'extra', label:'May include up to 5 additional Recon Legionaries:', mode:'add-models', scope:'unit', min:0, max:5,
        choices:[ {id:'recon', name:'Recon Legionary', cost:12, costMode:'per-model'} ]},
      { id:'bayonet', label:'Any model with a bolter may take one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[
          {id:'bayonet',       name:'Bayonet',       cost:1, costMode:'per-each'},
          {id:'chain_bayonet', name:'Chain bayonet', cost:2, costMode:'per-each'},
      ...
      { id:'vexilla', label:'One Recon Legionary may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'legion_vexilla', name:'Legion vexilla', cost:10, costMode:'flat'} ]},
      { id:'nuncio', label:'One Recon Legionary may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'nuncio_vox', name:'Nuncio-vox', cost:10, costMode:'flat'} ]},
      { id:'augury', label:'One Recon Legionary may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'augury_scanner', name:'Augury scanner', cost:10, costMode:'flat'} ]},
      { id:'unit_bombs', label:'The entire unit may take:', mode:'unit-per-model', scope:'each-model',
        choices:[ {id:'melta_bombs', name:'Melta bombs', cost:5, costMode:'per-model'} ]},
      { id:'sgt_bolter', label:'The Sergeant may exchange bolter for one of:', mode:'pick-one', scope:'sergeant',
  ...
  /* ── TR · Grey Slayer Pack ───────────────────────────────────────────────── */
  {
    id:'grey_slayer_pack', name:'Grey Slayer Pack', slot:'TR', baseCost:145,
    profileType:'model', composition:{start:10, min:10, max:20},
    profiles:[
      // Inv 6++ z Combat shield (stały wargear)
      {name:'Grey Slayer', M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:1, Ld:7, Sv:'3+', Inv:'6++', base:'32mm'},
      {name:'Huscarl',     M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:2, Ld:8, Sv:'3+', Inv:'6++', base:'32mm'},
    ],
    wargear:['Bolt pistol','Fenrisian axe','Combat shield','Power armour','Frag grenades','Krak grenades'],
    unitType:['Grey Slayer: Infantry (Skirmish, Line)','Huscarl: Infantry (Skirmish, Line, Character)'],
    rules:['Legiones Astartes (Space Wolves)','Relentless','Counter-attack (1)','Heart of the Legion'],
    rulesText:[],
    transportNote:'≤10 modeli: Rhino / Land Raider Proteus jako Dedicated Transport (nie zużywa slotu FOC; koszt płatny).',
    options:[
      { id:'extra', label:'May include up to 10 additional Grey Slayers:', mode:'add-models', scope:'unit', min:0, max:10,
        choices:[ {id:'grey_slayer', name:'Grey Slayer', cost:12, costMode:'per-model'} ]},
      { id:'shield_swap', label:'Any model may exchange combat shield for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        note:'Wymiana combat shield usuwa 6++ dla tego modelu.',
        choices:[ {id:'bolter', name:'Bolter', cost:0, costMode:'per-each', free:true} ]},
      { id:'bayonet', label:'Any model with a bolter may take one of:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[
          {id:'bayonet',       name:'Bayonet',       cost:1, costMode:'per-each'},
          {id:'chain_bayonet', name:'Chain bayonet', cost:2, costMode:'per-each'},
        ]},
      { id:'axe_swap', label:'Any model may exchange Fenrisian axe for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[ {id:'power_weapon', name:'Power weapon', cost:5, costMode:'per-each'} ]},
      { id:'axe_ratio', label:'For every five models, one may exchange Fenrisian axe for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1},
        choices:[
          {id:'heavy_chainsword', name:'Heavy chainsword', cost:2,  costMode:'per-each'},
          {id:'power_fist',       name:'Power fist',       cost:20, costMode:'per-each'},
          {id:'lightning_claw',   name:'Lightning claw',   cost:10, costMode:'per-each'},
        ]},
      { id:'pistol_ratio', label:'For every five models, one may exchange bolt pistol for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1},
        choices:[
          {id:'hand_flamer',   name:'Hand flamer',   cost:5,  costMode:'per-each'},
      ...
      { id:'nuncio', label:'One Grey Slayer may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'nuncio_vox', name:'Nuncio-vox', cost:10, costMode:'flat'} ]},
      { id:'vexilla', label:'One Grey Slayer may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'legion_vexilla', name:'Legion vexilla', cost:10, costMode:'flat'} ]},
      { id:'hus_melee', label:'The Huscarl may exchange Fenrisian axe for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'power_fist',     name:'Power fist',                cost:20, costMode:'flat'},
          {id:'lightning_claw', name:'Lightning claw',            cost:10, costMode:'flat'},
          {id:'frost_blade',    name:'Frost blade (sword/axe/claw)', cost:15, costMode:'flat'},
          {id:'thunder_hammer', name:'Thunder hammer',            cost:25, costMode:'flat'},
        ]},
      { id:'hus_pistol', label:'The Huscarl may exchange bolt pistol for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'hand_flamer',   name:'Hand flamer',   cost:5,  costMode:'flat'},
          {id:'plasma_pistol', name:'Plasma pistol', cost:10, costMode:'flat'},
        ]},
      { id:'hus_dual', label:'The Huscarl may exchange Fenrisian axe AND bolt pistol for:', mode:'toggle', scope:'sergeant',
        choices:[ {id:'two_lightning_claws', name:'Two lightning claws', cost:15, costMode:'flat'} ]},
      { id:'hus_armour', label:'The Huscarl may exchange power armour for:', mode:'single-swap', scope:'sergeant',
        choices:[ {id:'artificer_armour', name:'Artificer armour', cost:10, costMode:'flat', statMods:{Sv:'2+'}} ]},
      { id:'hus_bombs', label:'The Huscarl may take:', mode:'toggle', scope:'sergeant',
        choices:[ {id:'melta_bombs', name:'Melta bombs', cost:10, costMode:'flat'} ]},
    ],
  },

  /* ── TR · Grey Stalker Pack ──────────────────────────────────────────────── */
  {
    id:'grey_stalker_pack', name:'Grey Stalker Pack', slot:'TR', baseCost:145,
    profileType:'model', composition:{start:10, min:10, max:20},
    profiles:[
      {name:'Grey Stalker', M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:1, Ld:7, Sv:'3+', Inv:'—', base:'32mm'},
      {name:'Huscarl',      M:7, WS:4, BS:4, S:4, T:4, W:1, I:4, A:2, Ld:8, Sv:'3+', Inv:'—', base:'32mm'},
    ],
    wargear:['Bolt pistol','Chainsword','Power armour','Frag grenades','Krak grenades'],
    unitType:['Grey Stalker: Infantry (Skirmish, Line)','Huscarl: Infantry (Skirmish, Line, Character)'],
    rules:['Legiones Astartes (Space Wolves)','Relentless','Counter-attack (1)','Heart of the Legion'],
    rulesText:[],
    transportNote:'≤10 modeli: Rhino / Land Raider Proteus jako Dedicated Transport (nie zużywa slotu FOC; koszt płatny).',
    options:[
      ...
      { id:'chain_swap', label:'Any model may exchange chainsword for:', mode:'ratio-swap', scope:'model', ratio:{per:1,count:1},
        choices:[ {id:'power_weapon', name:'Power weapon', cost:5, costMode:'per-each'} ]},
      { id:'special', label:'For every five models, one may take one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1},
        choices:[
          {id:'magna_combi',    name:'Magna combi-weapon', cost:10, costMode:'per-each'},
          {id:'minor_combi',    name:'Minor combi-weapon', cost:5,  costMode:'per-each'},
          {id:'flamer',         name:'Flamer',             cost:5,  costMode:'per-each'},
          {id:'volkite_charger',name:'Volkite charger',    cost:5,  costMode:'per-each'},
          {id:'rotor_cannon',   name:'Rotor cannon',       cost:5,  costMode:'per-each'},
          {id:'volkite_caliver',name:'Volkite caliver',    cost:5,  costMode:'per-each'},
          {id:'plasma_gun',     name:'Plasma gun',         cost:10, costMode:'per-each'},
          {id:'meltagun',       name:'Meltagun',           cost:15, costMode:'per-each'},
        ]},
      { id:'melee_ratio', label:'For every five models, one may exchange chainsword for one of:', mode:'ratio-swap', scope:'model', ratio:{per:5,count:1},
      ...
      { id:'nuncio', label:'One Grey Stalker may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'nuncio_vox', name:'Nuncio-vox', cost:10, costMode:'flat'} ]},
      { id:'vexilla', label:'One Grey Stalker may take:', mode:'toggle', scope:'unit',
        choices:[ {id:'legion_vexilla', name:'Legion vexilla', cost:10, costMode:'flat'} ]},
      { id:'hus_melee', label:'The Huscarl may exchange chainsword for one of:', mode:'pick-one', scope:'sergeant',
        choices:[
          {id:'power_fist',     name:'Power fist',                cost:20, costMode:'flat'},
          {id:'lightning_claw', name:'Lightning claw',            cost:10, costMode:'flat'},
          {id:'frost_blade',    name:'Frost blade (sword/axe/claw)', cost:15, costMode:'flat'},
      ...
      { id:'hus_dual', label:'The Huscarl may exchange chainsword AND bolt pistol for:', mode:'toggle', scope:'sergeant',
        choices:[ {id:'two_lightning_claws', name:'Two lightning claws', cost:15, costMode:'flat'} ]},
      { id:'hus_armour', label:'The Huscarl may exchange power armour for:', mode:'single-swap', scope:'sergeant',
        choices:[ {id:'artificer_armour', name:'Artificer armour', cost:10, costMode:'flat', statMods:{Sv:'2+'}} ]},
      { id:'hus_bombs', label:'The Huscarl may take:', mode:'toggle', scope:'sergeant',
